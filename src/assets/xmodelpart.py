...
"""
XModelPart represents an xmodelpart structure
"""
class XModelPart:

    class VIEWHANDS(metaclass = enum.BaseEnum):
        COD1 = {
    ...
    __slots__ = ('name', 'version', 'model_type', 'bones')
    ...
    def _load_v14(self, file: bytes) -> bool:
        header = file_io.read_fmt(file, '2H', collections.namedtuple('header', 'bone_count, root_bone_count'))
        ...
        for bone_index in range((header.root_bone_count + header.bone_count)):
            ...
            if self.model_type == xmodel.TYPES.VIEWHANDS and  bone_name in self.VIEWHANDS.COD1:
                local_viewmodel_position = self.VIEWHANDS.COD1[bone_name]
    ...
    def _load_v20(self, file: bytes) -> bool:
        ...
        for bone_index in range((header.root_bone_count + header.bone_count)):
            ...
            # set viewmodel positions
            if self.model_type == xmodel.TYPES.VIEWHANDS and  bone_name in self.VIEWHANDS.COD2:
                local_viewmodel_position = self.VIEWHANDS.COD2[bone_name]
                world_viewmodel_position = copy.deepcopy(local_viewmodel_position)
                current_bone.local_transform.position = local_viewmodel_position / 2.54
                current_bone.world_transform.position = world_viewmodel_position / 2.54
        ...
        try:
            with open(xmodel_part, 'rb') as file:
                ...
                if version not in xmodel.VERSIONS:
                    log.info_log(f'xmodelpart version {version} is not supported')
                    return False
